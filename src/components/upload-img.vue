...
const imageUrl = ref([]);
const { imgUrl, index, imgType } = toRefs(props);
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    if (newVal.indexOf("|") == -1) {
      ...
      imageUrl.value = newVal.split("|").filter((el) => {
        return el != "";
      });
    }
  },
...
/*压缩图片
      ...
      callback：容器或者回调函数
      */
function photoCompress2(file, obj, callback) {
...
/* 利用canvas数据化图片进行压缩 */
/* 图片转base64 */
function canvasDataURL2(path, obj, callback) {
  let img = new Image();
  img.src = path;
  img.onload = function () {
    let that = this; // 指到img
...
function base64UrlToBlob2(urlData) {
...
// 返回file文件，调用接口执行上传
async function uploadLice2(file) {
  console.log("圖片file", file);
  let formdata = new FormData(); //创建form对象
  formdata.append("files", file); //通过append向form对象添加数据
  formdata.append("type", imgType.value);

  const { data: res } = await Api.uploadFile(formdata);
...
// 刪除圖片
function delImage(item) {
  console.log(item);
  for (const i in imageUrl.value) {
