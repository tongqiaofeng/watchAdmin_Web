  ...
  <div class="sale-history-container" id="saleHistory">
    ...
    <div v-show="pageSel === 0">
      <div class="back-style">
        <el-form inline>
          <el-form-item label="接收仓库：" prop="receiveWarehouse">
            <el-cascader
              size="large"
              style="width: 100%"
              v-model="checkData.receiveWarehouse"
              :options="consignWarehouseList"
              :props="{
                value: 'id',
                label: 'name',
      ...
      <div v-else style="margin-top: 20px">
        ...
        >
          <div class="bill-container">
            <div class="container-top-left">
              ...
              <div style="margin-left: 30px; font-size: 16px; color: #000">
                寄卖时间：<span style="font-size: 14px; color: #666">{{
                  item.stockSellTime
                }}</span>
              </div>
              <div style="margin-left: 30px; font-size: 16px; color: #000">
                接收仓库：<span style="font-size: 14px; color: #666">{{
                  item.receiveCompanyName + " - " + item.receiveWarehouseName
                }}</span>
              </div>
            </div>
            <div class="print">
          ...
          <el-table border stripe :data="item.watchList" style="width: 100%">
            ...
            <el-table-column
              ...
              label="机芯号"
            ></el-table-column>
            <el-table-column align="center" prop="sellMoney" label="寄卖金额">
              <template #default="scope">
                <div>
                  {{
                    scope.row.sellMoney
        ...
        >
          ...
          <template #footer>
            <div>
              <el-button @click="dialogDeleteSaleVisible = false"
                >取 消</el-button
              >
              <el-button type="primary" v-preventClick @click="cancelSureSales"
                >确 定</el-button
              >
            </div>
          </template>
...
// 获取已有销售单接收仓库列表
const consignWarehouseList = ref([]);
const getConsignmentWarehouseList = async () => {
  const { data: res } = await Api.consignWarehouseList();
  console.log("寄卖接收仓库列表");
  console.log(res);
  consignWarehouseList.value = res.data;
};
...
const getsaleOrderList = async () => {
  const { data: res } = await Api.sellOrderSearch({
    ...checkData.value,
    receiveWarehouseId: checkData.value.receiveWarehouse
      ? checkData.value.receiveWarehouse[1]
      : null,
